package app;

public class Constant {
    public static final int BOARD_WIDTH = 360;
    public static final int BOARD_HEIGHT = 640;

    public static final int DEFAULT_BIRD_X = BOARD_WIDTH / 8;
    public static final int DEFAULT_BIRD_y = BOARD_HEIGHT / 2;
    public static final int DEFAULT_PIPE_X = BOARD_WIDTH;
    public static final int DEFAULT_PIPE_Y = 0;

    public static final int BIRD_WIDTH = 34;
    public static final int BIRD_HEIGHT = 24;

    public static final int PIPE_WIDTH = 64;
    public static final int PIPE_HEIGHT = 512;

    public static final int GRAVITY = 1;
    public static final int FRAME_PER_SECOND = 60;
}
